...
for start_point in range(0, len(u), time_to_simulate):  # Start at data point 10, increment by 10 until the end of the data
    ...
    plt.clf()
    plt.subplot(2, 1, 1)
    plt.title('ARX Model Depth Rate Validation', fontsize=titlesize , fontweight='bold')
    ...
    plt.subplot(2, 1, 2)
    plt.plot(t_sim_forplot, y[output1][0:start_point+time_steps], 'm-',
            label='Measured')
    plt.plot(t_sim_forplot, ypred_forplot, 'k:',
            label='Estimated')

    plt.ylabel('Depth Rate (mm/s)', fontsize=fontsize)
