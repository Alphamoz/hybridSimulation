...
u = u[startData:150].reset_index(drop=True)
y = y[startData:150].reset_index(drop=True)
t = t[0:u.shape[0]]
...
for start_point in range(0, math.floor(len(u)/time_to_simulate)*time_to_simulate, time_to_simulate):  # Start at data point 10, increment by 10 until the end of the data
    ...
    u_sim2_forplot.extend(u_sim[input2])
    t_sim_forplot.extend(np.linspace(start_point, start_point+time_steps-1,time_steps))
    print(ypred[0:,0],y[output1][0:start_point+time_steps])
    # getting MSE
    mse = np.mean((ypred[0:,0] - y[output1][start_point:start_point+time_steps])**2, axis=0)
    mse_total = np.append(mse_total,mse)
    print("Total error (MSE):", np.mean(mse_total))
    ...
    plt.clf()
    plt.subplot(2, 1, 1)
    plt.title('ARX Model Pitch Validation', fontsize=titlesize , fontweight='bold')
    plt.plot(t_sim_forplot, u_sim_forplot, 'b-', label=r'$u_{br}$')
    plt.plot(t_sim_forplot, u_sim2_forplot, 'g-', label=r'$u_{mr}$')
    ...
    plt.subplot(2, 1, 2)
    plt.plot(t_sim_forplot, y[output1][0:start_point+time_steps], 'r-',
            label='Measured')
    plt.plot(t_sim_forplot, ypred_forplot, 'k:',
            label='Estimated')
    ...
    plt.ylabel('Pitch (degree)', fontsize=fontsize)
