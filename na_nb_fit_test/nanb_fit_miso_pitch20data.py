...
for na in na_s:
    for nb in nb_s:
        # be, depth rate model
        u,y,t = initializeData(data='./data/dataProcessing/processedData/DataModel5detik.csv',inputs = ["be_rate","mm_rate"], outputs = ["pitch"], time=["Time"],startData=0, endData=-1, normalization=0, zeroing=True, scaleOutput=0)
        ...
        mse_data.to_csv('mse_values_train_pitch20.csv', mode='w' if first==True else 'a', header=(True if first==True else False), index=False)
        ...
        print(mse_dict)
        mse_data = pd.DataFrame(mse_dict)
        mse_data.to_csv('mse_values_test_pitch20.csv', mode='w' if first==True else 'a', header=(True if first==True else False), index=False)
        first = False 
