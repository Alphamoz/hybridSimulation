...
for i,ctrlhor in [[100,10],[100,15],[100,20],[100,30],[100,40]]:
    MPC_no_Disturbance_Data = np.load('simulation_data_MPCnoDisturbance{}{}.npz'.format(i, ctrlhor))
    ...
    os = calculate_overshoot(loaded_depth_sim,20000)
    steady_error = calculate_steady_error(loaded_depth_sim,20000)
    print("Depth for {}".format(ctrlhor), settling_time_depth, rise_time_depth, energy, os, steady_error)
    ...
    print("Pitch for {}".format(ctrlhor),settling_time_pitch, rise_time_depth, energy, os, steady_error)

    settling_time_pitch = calculate_settling_time(loaded_depth_rate_sim, i, 0.05)
    rise_time_depth = calculate_rise_time(loaded_depth_rate_sim, i, 0.9)
    os = calculate_overshoot(loaded_depth_rate_sim, i)
    steady_error = calculate_steady_error(loaded_depth_rate_sim, i)

    print("Depth Rate for {}".format(ctrlhor),settling_time_pitch, rise_time_depth, energy, os,steady_error)
