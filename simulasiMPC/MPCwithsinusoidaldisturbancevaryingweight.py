...
for i in range(1, num_steps):
    ...
    weight_varying = True
    if(weight_varying):
        if(abs(setpointdepthrate-depth_rate.MEAS)<40):
            R1=10
        if(abs(pitch_angle.SP-pitch_angle.VALUE[1])<10):
            R2=1
        if(abs(setpointdepthrate-depth_rate.MEAS)<10):
            R1=1000
    ...
    # plt.figsize()
    plt.subplot(4, 1, 1)
    plt.title("MPC Control Gliding Simulation with Disturbance and Adaptive Weight".format())
...
file_path = "simulation_data_MPCDisturbanceweightvarying{}.npz".format(weight_varying)
