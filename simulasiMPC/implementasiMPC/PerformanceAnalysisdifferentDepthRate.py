...
for first1,first2,last1,last2 in [[0,0,501,5027]]:
    data = pd.read_csv('./data/lastexpguiupdated.csv')
    # setpoint data
    depth_rate_sp = data["field.depth_rate_sp"][first1:last1]
    pitch_sp = data["field.pitch_sp"][first1:last1]
    depth_sp = data["field.depth_sp"][first1:last1]
    time1 = data["time1"]
    time2 = data["time2"]
    print(np.argmin(np.array(time1)<500))
    print(np.argmin(np.array(time2)<500))
    
    loaded_depth_sim = data["field.depth_data"][first2:last2]
    loaded_pitch_angle_sim = data["field.pitch_data"][first2:last2]
    # print(loaded_pitch_angle_sim)
    
    loaded_BE = data["inputbe"][first2:last2]
    loaded_MM = data["inputmm"][first2:last2]
