...
# depth2 = m.Var(value=0)
# depth_rate2 = m.CV(value=0)
...
# m2.Equation(depth2 == m2.integral(depth_rate2))
# m2.Equation(BE2 == m2.integral(br))
# m2.Equation(MM2 == m2.integral(mr))

m.Equation(depth > 0)

pitch_angle = m2.CV(value=0)
...
m2.Minimize(1*((pitch_angle - pitch_angle.SP))**2)


# Define the dynamic equations
m.arx(p_dr, y=[depth_rate], u=[br])
m2.arx(p_pitch, y=[pitch_angle], u=[br2, mr2])
...
for i in range(1, num_steps-1):
    try:
        ...
        solvetime1 = m.options.SOLVETIME
        m2.solve(disp=True, debug=1)
        solvetime2 = m.options.SOLVETIME
        solvingtime.append(solvetime1 + solvetime2)
    except:
    ...
    m._objectives.clear()
    m2._objectives.clear()
    m.Minimize(Q1*((depth_rate - setpointdepthrate)/100)**2)
    m2.Minimize(Q2*((pitch_angle - pitch_angle.SP)/20)**2)
    m.Minimize(R1*(br/15)**2 + R2*(mr/10)**2)
    # m2.Minimize(R1*(br/15)**2 + R2*(mr/10)**2)
...
# # Plot the results
# plt.savefig('MPCwoDisturbanceParamWeight{}.png'.format(
#     weight), dpi=300)
# plt.show()
...
# file_path = "simulation_data_MPCnoDisturbance{}{}{}{}.npz".format(Q1,Q2,R1,R2)
